...
class Context {
    ...
    constructor(options = {}, scope = {}, parentId = null) {
        ...
        this.receive = this.makeReceiver();
        this.emit    = this.makeEmitter();

        this.emit.begin();
    }

    /**
     * Generate a "receiver" function that will handle (either parse or log) emitted event objects
     * @returns {function}
     */
    makeReceiver() {
        if (this.options.cli && !this.options.raw) {
            return makeFormattingReceiver(this.constructor.eventFormatters, this.options);
        } else {
            return obj => console.log(JSON.stringify(obj));
        }
    }

    /**
     * Generate an "emitter" (an object that has methods to emit each of the events made available by the eventfactories)
     * @returns {function}
     */
    makeEmitter() {
        return makeEmitter(this.constructor.eventFactories, this, this.receive);
    }
