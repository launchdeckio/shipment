...
class Shipment {
    ...
    constructor(actions = [], options = {}, exportPath = null) {
        ...
        this.actions    = Shipment.transformActions(actions);
        this.exportPath = exportPath;
    }

    /**
     * Transform all given actions into BaseAction instances. And assert that they all have valid identifiers.
     * @param {Object|Class[]|Function[]|{BaseAction}[]} actions
     * @returns {Array}
     */
    static transformActions(actions) {
        return _.map(actions, (action, key) => {
            action = transformAction(action);
            if (_.isString(key)) action.getName = () => _.kebabCase(key);
            else if (!action.getName())
                throw new Error('Couldn\'t determine an appropriate name for this action. ' +
                    '(did you declare it as an anonymous function using arrow syntax?)');
            return action;
        });
    }
    ...
    /**
     * Run HTTP server
     * @param {Object} options Options for the ShipmentServer, see ShipmentServer constructor
     * @returns {http.Server}
     */
    serve(options) {
