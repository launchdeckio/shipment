...
const rawFormatter     = require('./rawFormatter');
...
class CliWrapper {
    ...
    getHandler(actionNam, meta) {
        ...
        return argv => {
            ...
            return action.execute(args, receive).catch(() => {
                process.exit(1);
            });
        };
    }
    ...
    getFormatter(args) {
        const middleware = [
            formatterToMiddleware(defaultFormatter),
            ...this.middleware,
            formatterToMiddleware(rawFormatter)
        ].map(mf => mf(args));
