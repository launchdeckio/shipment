...
const reportCli   = require('./parse/reportCli');
...
class Reporter {
    ...
    constructor(contextId, options = {}) {
        ...
        if (options.cli && !options.raw) {
            this.parser = new EventParser();
            this.reportCli(this.parser, options);
     ...
     * @param {Object} options
     */
    reportCli(parser, options) {
        reportCli(parser, options);
        // Invoke all functions in the static cliReporters property with parser, options
        _.forEach(arrify(this.constructor.cliReporters), cliReporter => cliReporter(parser, options));
    }
