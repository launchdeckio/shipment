...
class ShipmentServer {
     ...
     * @param {Number} port The port on which to listen for incoming connections (default SHIPMENT_PORT env)
     * @param {Number} basePath The base route (default SHIPMENT_BASE_PATH env)
     * @param {Boolean} [useFork = false] Whether to use a forked process (child process) to run the action
    ...
    constructor(shipment, {
        ...
        output = true,
        useFork = false,
        port = process.env.SHIPMENT_PORT || 6565,
    ...
    static get workerPath() {
        return path.join(__dirname, 'fork.js');
    }
