...
class Action {
    ...
    makeContext(args, cli = false, parent = null, optionOverrides = {}) {
        ...
        // Determine whether just a parent ID was passed in, or a complete Context instance
        const isParentRealContext = parent instanceof Context;

        // When dealing with a Context instance, retrieve the inherited options
        const inheritedOptions = isParentRealContext ? parent.getInheritedOptions() : {};
        const options          = extend({}, inheritedOptions, optionsFromArgs, {cli}, optionOverrides);
        const scope            = this.prepareScope();
        const parentId         = isParentRealContext ? parent.id : parent;
