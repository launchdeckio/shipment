...
class Action {
    ...
    makeContext(args, cli) {
        return new (this.constructor.Context)(_.extend({}, this.parseContextArgs(args), {cli: !!cli}));
    }
    ...
    executeCli(args) {
        ...
        return exec().then(() => this.onSuccess(context), e => this.onError(context, e));
    }
    ...
    parseContextArgs(args) {
        ...
        _.forEach(this.getAvailableOptions(), option => option.transformContextOptions(options, args));
        return options;
    }
    ...
    register(yargs) {
        return yargs.command(
            ...
            args => Promise.resolve(this.executeCli(args)).done()
        );
    }
}
