...
class Action {
     ...
     * @param {Context} context
     * @param {Error} error
     * @param {Boolean} [rethrow = true] Whether to rethrow the error
     */
    onError(context, error, rethrow = true) {
        context.report('fatal', {error: error.message});
        if (rethrow) throw error;
    }

    /**
     * Return a function that will execute the action within the scope of the action
     * @param {Context} context
     * @param {Object} args
     * @return {Function}
     */
    prepare(context, args) {
        ...
        return () => context.withScope(scope, context => this.execute(context, args));
    }
    ...
    execute(context, args) {
        ...
        // Invoke the main "run" method
            ...
            .catch(e => this.onError(context, e, true))
     ...
     * @returns {Promise}
     */
    executeCli(args, exitCodeOnError) {
        return Promise.resolve(this.prepare(this.makeContext(args, true), args)())
            .catch(e => {
     ...
     */
    executeApi(args = {}) {
        return this.prepare(this.makeContext(args, false), args)();
    }
